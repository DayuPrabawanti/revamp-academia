...
func (q *Queries) ListUsers(ctx context.Context) ([]usersModel.UsersUser, error) {
	rows, err := q.db.QueryContext(ctx, listUsers)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []usersModel.UsersUser
	for rows.Next() {
		var i usersModel.UsersUser
		if err := rows.Scan(
			&i.UserEntityID,
			&i.UserName,
			&i.UserPassword,
...
// users
func (q *Queries) GetUser(ctx context.Context, userEntityID int32) (usersModel.UsersUser, error) {
...
func (q *Queries) CreateUsers(ctx context.Context, arg CreateUsersParams) (*usersModel.UsersUser, *usersModel.ResponseError) {
	row := q.db.QueryRowContext(ctx, createUsers,
		arg.UserEntityID,
		arg.UserName,
		arg.UserPassword,
	...
	return &usersModel.UsersUser{
		...
		UserModifiedDate:        sql.NullTime{Time:time.Now(), Valid:true},
