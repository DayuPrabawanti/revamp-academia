...
// curriculum.sections
func (q *Queries) Getsections(ctx context.Context, sectID int32) ([]curi.CurriculumSectionGet, error) {
	...
	var result []curi.CurriculumSectionGet
	for rows.Next() {
		var i curi.CurriculumSectionGet
		err := rows.Scan(
			&i.SectProgEntityID,
			&i.SectTitle,
			&i.SectDescription,
