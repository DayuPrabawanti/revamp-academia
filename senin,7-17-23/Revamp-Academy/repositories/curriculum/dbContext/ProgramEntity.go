...
func (q *Queries) Getprogram_entity(ctx context.Context, progEntityID int32) (curi.CurriculumProgramEntity, error) {
	row := q.db.QueryRowContext(ctx, getprogram_entity, progEntityID)
	var i curi.CurriculumProgramEntity
	err := row.Scan(
		&i.ProgEntityID,
		&i.ProgTitle,
		&i.ProgHeadline,
...
func (q *Queries) Listprogram_entity(ctx context.Context) ([]curi.CurriculumProgramEntity, error) {
	rows, err := q.db.QueryContext(ctx, listprogram_entity)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []curi.CurriculumProgramEntity
	for rows.Next() {
		var i curi.CurriculumProgramEntity
		if err := rows.Scan(
			&i.ProgEntityID,
...
func (q *Queries) ListMasterCategories(ctx context.Context) ([]curi.MasterCategory, error) {
	rows, err := q.db.QueryContext(ctx, listMasterCategories)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []curi.MasterCategory
	for rows.Next() {
		var i curi.MasterCategory
		if err := rows.Scan(
			&i.CateID,
			&i.CateName,
			&i.CateCateID,
