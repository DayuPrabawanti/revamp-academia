...
func (q *Queries) ListUsers(ctx context.Context) ([]usersm.UsersUser, error) {
	rows, err := q.db.QueryContext(ctx, listUsers)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []usersm.UsersUser
	for rows.Next() {
		var i usersm.UsersUser
		if err := rows.Scan(
			&i.UserEntityID,
			&i.UserName,
			&i.UserPassword,
...
// users
func (q *Queries) GetUser(ctx context.Context, userEntityID int32) (usersm.UsersUser, error) {
...
func (q *Queries) CreateUsers(ctx context.Context, arg CreateUsersParams) (*usersm.UsersUser, *usersm.ResponseError) {
	row := q.db.QueryRowContext(ctx, createUsers,
		arg.UserEntityID,
		arg.UserName,
		arg.UserPassword,
