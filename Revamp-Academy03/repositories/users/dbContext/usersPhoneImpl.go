...
func (q *Queries) ListPhones(ctx context.Context) ([]usersModel.UsersUsersPhone, error) {
	rows, err := q.db.QueryContext(ctx, listPhones)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []usersModel.UsersUsersPhone
	for rows.Next() {
		var i usersModel.UsersUsersPhone
		if err := rows.Scan(
			&i.UspoEntityID,
			&i.UspoNumber,
			&i.UspoModifiedDate,
...
// Users Phones
func (q *Queries) GetPhones(ctx context.Context, uspoEntityID int32) (usersModel.UsersUsersPhone, error) {
...
RETURNING *
`
func (q *Queries) CreatePhones(ctx context.Context, arg CreatePhonesParams) (*usersModel.UsersUsersPhone, *usersModel.ResponseError) {
	...
	return &usersModel.UsersUsersPhone{
