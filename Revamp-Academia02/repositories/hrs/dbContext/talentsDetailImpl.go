...
func (q *Queries) ListTalentsDetail(ctx context.Context) ([]hrsMdl.TalentsDetailMockup, error) {
	rows, err := q.db.QueryContext(ctx, listTalentsDetail)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []hrsMdl.TalentsDetailMockup
	for rows.Next() {
		var i hrsMdl.TalentsDetailMockup
