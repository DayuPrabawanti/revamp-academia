...
func (paymentFintechController PaymentFintechController) CreateNewPaymentFintech(ctx *gin.Context) {
	...
	var category dbContext.CreatePaymentFintechParams
	...
	response, responseErr := paymentFintechController.paymentFintechService.CreateNewPaymentFintech(ctx, &category)
	if responseErr != nil {
...
func (paymentFintechController PaymentFintechController) DeletePaymentFintechById(ctx *gin.Context) {
	...
	responseErr := paymentFintechController.paymentFintechService.DeletePaymentFintechById(ctx, int64(categoryId))
	if responseErr != nil {
		ctx.AbortWithStatusJSON(responseErr.Status, responseErr)
		return
	}
