...
import (
	...
	userservices "codeid.revampacademy/services/userServices"
	"github.com/gin-gonic/gin"
)

type UserEmailController struct {
	userEmailService *userservices.UserEmailService
}

// Declare constructor
func NewUserEmailController(userEmailService *userservices.UserEmailService) *UserEmailController {
...
func (userEmailController UserEmailController) GetUserEmail(ctx *gin.Context) {
	...
	response, responseErr := userEmailController.userEmailService.GetUserEmail(ctx, int32(emailId))
	if responseErr != nil {
...
func (useremailcontroller UserEmailController) CreateUserEmail(ctx *gin.Context) {
	...
	response, responseErr := useremailcontroller.userEmailService.CreateUserEmail(ctx, &email)
	if responseErr != nil {
